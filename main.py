...
def place_order(symbol, side, size, leverage, tp_list, sl_price):
    ...
    try:
        ...
        return {"error": f"❌ Invalid JSON response from BloFin: {e}\nRaw: {r.text}"}
...
async def on_message(message):
    ...
    if not all([symbol_match, buyzone_match, targets, stop_match, lev_match]):
        print("⚠️ Missing required trade data — skipping")
        return
    ...
    market_price = tp_list[0]  # use first TP just for reference sizing
    size = round((TRADE_AMOUNT * leverage) / market_price, 3)

    order = place_order(symbol, 1, size, leverage, tp_list, sl_price)

    if order.get("code") == "0":
        await message.channel.send(f"✅ Trade Placed: {symbol} | Size: {size}")
    else:
